...
BarRep[Lorentz]:=Lorentz;
BarRep[Spin]:=Spin;
BarRep[Spin1]:=Spin1;
BarRep[Spin2]:=Spin2;

BarRep[Index[x_]]:=Index[BarRep[x]];
BarRep[Index[x_,y_]]:=Index[BarRep[x],y];
...
DeclareNewDefinition[rule_, outputlist_]:=Block[{tempdef = rule,tempdeflhs,tempdefrhs,MyModule,modulelist,tempspinindex,indexname, newoutputlist, MyRuleDelayed,sumspin},
   ...
   If[Not[FreeQ[tempdeflhs, Ga[0,___]]],       
    ...
    (*tempdefrhs = Replace[tempdefrhs(ma:ProjP|ProjM|_TensorDot|_Ga)[a_, sp1_]*b_[a_, ff___]*c_:>MyRelease[Hold[Module[{xx},c*ma[xx,sp1]*b[xx,ff]]]]];*)
...
Options[LoadModel] = {Output -> MR$Default, ClassName -> MR$Null, ClassMembers -> {}, FlavorIndex -> MR$Null, QuantumNumbers -> {},
                      ...
                      RunningParameter->False,PyrateTag->MR$Null,GaugeField->False};
...
(* End BF *)
   ...
   (*END BF *)
    ...
    (* update numQ and CnumQ and parameters *)
    MakeNumQ[EParamList, IParamList];
    FR$RunningParameters = Select[MR$ParameterList,RunningParameter[#]&];
    ...
    (*Update*)
     ...
     (*Initialize the bar representation*)
     InitBarRep[MR$IndexList];
     UnfoldIndices[];
...
AddDefinition[Rule[x_,y_]|RuleDelayed[x_,y_],options___]:=Block[{temprule1,temprule2,temprule3,temprule4,flavexp,indlist,flavrules={},ii,kk,
  tempipl=IParamList,
  ...
  (*expand indices for rules involving tensors with symbolic indices*)
  If[Length[x]>0&&Head[NumericalValue[x]]===NumericalValue,
	flavexp={x};
	If[FreeQ[M$Parameters,Head[x] ], 
		Print[Style["The restriction on the following parameter cannot be done as the parameter has not be defined",Red]];
		Print[InputForm[x]];
		Abort[];];
	indlist=Indices/.M$Parameters[[Position[M$Parameters,Head[x]][[1,1]],2]];
	For[ii=1,ii<=Length[indlist],ii++,
		If[Not[MatchQ[x[[ii]],_Integer|Append[indlist[[ii]],_Integer]]],
			For[kk=1,kk<=Length[flavexp],kk++,
				flavexp[[kk]]=((tmp=flavexp[[kk]];tmp[[ii]]=Append[indlist[[ii]],#];tmp)&)/@IndexRange[indlist[[ii]]];
			];
			flavexp=Flatten[flavexp];
		];
	];
	flavrules={(#->(#/.Union[{RuleDelayed[x,y],temprule1,temprule2,temprule3,temprule4}])&)/@flavexp};
	flavrules = Join[flavrules,(flavrules/.ParamRules),(flavrules/. Index[_, k_] :> k)];
  ];
  
  (*tempipl=DeleteCases[tempipl,_?(((#[[1]]//.temprule2)=!=#[[1]])||((#[[1]]//.RuleDelayed[x,y])=!=#[[1]])&)];*)
  tempipl = MapAt[# //. RuleDelayed[x,y] //. temprule4 //.flavrules&, #, 2]& /@ tempipl;
  
  (*IParamList=tempipl/.ParamRules;
  
  Print[InputForm[tempipl]];
  Print[InputForm[ParamRules]];*)
...
(* ::Text:: *)
(*Initialise bar representation of indices*)


InitBarRep[indlist_List] := (DefineBarRep/@indlist);

DefineBarRep[Index[zz_]]:= If[HasBarRepQ[zz]===True,
  BarRep[zz]=ToExpression[ToString[zz]<>"bar"];
  BarRep[Evaluate[BarRep[zz]]]=zz;
  IsBarRepQ[zz]=False;
  IsBarRepQ[BarRep[zz]]=True;
  (*IndexRange of the bar representation should not be defined, check it and set it to the same as the representation*)
  If[IndexRange[Index[BarRep[zz]]]=!=IndexRange[Index[BarRep[zz]]],
    Print[Style["Warning : the IndexRange of the Bar represation should not be defined",Orange]];,
    IndexRange[Index[BarRep[zz]]]=IndexRange[Index[zz]];
  ]
  (*Define the style of the bar representation to be the same are the original representation*)
  IndexStyle[Index[BarRep[zz]],ToExpression[StringTake[ToString[Index[SU2D,Ext[1]],StandardForm],{4}]]];,
  (*Has no Bar Rep*)
  If[IsBarRepQ[zz]=!=True,
    BarRep[zz]=zz;
    IsBarRepQ[zz]=False;
  ];
];
...
(*                   *)
       ...
       If[tempind =!= MR$Null, MR$IndexList = KillDoubles[Join[MR$IndexList, tempind]];InitBarRep[MR$IndexList];];
    ...
    If[ValueQ[M$GaugeGroups] && (Length[M$GaugeGroups] != 0),
      ...
      If[Not[repo], Print["   - Loading gauge group classes."]];
       ...
       Do[currentclass = MR$GaugeGroupList[[kk]];
          If[(PyrateTag/.MR$GaugeGroupRules[currentclass] /. Options[LoadModel])=!=MR$Null, PyrateTag[currentclass]=ToString[PyrateTag/.MR$GaugeGroupRules[currentclass]]];
          tempabelian1 = Abelian /. MR$GaugeGroupRules[currentclass] /. Options[LoadModel];
          If[tempabelian1,PyrateTag[currentclass]="U1"];
    ...
    (*                                                                           *)     
      If[ValueQ[M$Parameters] && (Length[M$Parameters] != 0),
         ...
         (*                                                   *)
               ...
               RunningParameter[currentclass] = RunningParameter/.MR$ParameterRules[currentclass] /. Options[LoadModel];
...
AddParticlesToClass[part_List, class_[n_], options___] := Block[{nind, classname, flav, addflav = True},
      ...
      MR$FlavorList = If[Not[MemberQ[MR$FlavorList, flav]] && Not[flav === MR$NoFlavorIndex], Join[MR$FlavorList, Union[{flav,BarRep[flav]}]], MR$FlavorList];
        (*                                  *)
...
(*Index declarations *)
      ...
      $IndList[anti[classname]] = (BarRep/@$IndList[classname]) /. {Spin1 -> Spin2, Spin2 -> Spin1};
      ...
      Do[$IndList[part[[kk]]] = If[FlavoredQ[classname], DeleteCases[$IndList[classname], Index[flav]], $IndList[classname]];
         $IndList[anti[part[[kk]]]] = (BarRep/@$IndList[part[[kk]]]) /. {Spin1 -> Spin2, Spin2 -> Spin1};
...
FormatGaugeMatrix[repmat_List, name_, adjind_] := Block[{temptensclass}, 
         ...
         DeclareTensor[repmat[[1]], {adjind, repmat[[2]], BarRep[repmat[[2]]]}, TensorClass -> temptensclass];
