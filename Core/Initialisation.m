...
DCint[group_][field_?(FieldQ[#] === True&), mu_,side_:Automatic] := Block[{
    ...
    charge,complexconjugate, dcii, dcres,newside
   ...
   (* Read out the basic field, i.e., without indices or derivatives *)
    ...
    charge = Intersection[fieldindices, Flatten[({#,BarRep[#]}&)/@(Last /@ rep)]];

     (*If the particle is not charge, stop *)
    If[charge=={},Return[0]];

    (* Check if the is at most one representation for each aprticle *)
    If[Length[Union[charge]]<Length[charge], 
       Message[DC::Rep2];Abort[]; 
      ];

    dcres=0;
    (*If[Length[charge]===1, (*if only one charge, conjugate and transposed*)
      If[IsBarRepQ[charge[[1]]],
        complexconjugate=True;
        repii = Cases[rep,{_,BarRep[charge[[1]]]}][[1]];repii[[2]] = charge[[1]],
        If[HasBarRepQ[charge[[1]]],complexconjugate=False];
        repii = Cases[rep,{_,charge[[1]]}][[1]];
      ];

      If[FieldComponentQ[field//.del[ff_,_]:>ff] === True,
        Print["compo"];dcres=dcres+ContstructNonAbelianFieldComponentDC[group,field,complexconjugate,fieldindices,repii,coup,boson,mu],
        Print["nocompo"];dcres=dcres+ContstructNonAbelianNonFieldComponentDC[group,field,complexconjugate,fieldindices,repii,coup,boson,mu,side]],*)
      For[dcii=1,dcii<=Length[charge],dcii++,
        If[IsBarRepQ[charge[[dcii]]],
          complexconjugate=True;newside=Left;repii = Cases[rep,{_,BarRep[charge[[dcii]]]}][[1]];repii[[2]] = charge[[dcii]],
          If[HasBarRepQ[charge[[1]]],complexconjugate=False,newside=Right];
          repii = Cases[rep,{_,charge[[dcii]]}][[1]];]

        If[FieldComponentQ[field//.del[ff_,_]:>ff] === True,
          dcres=dcres+ContstructNonAbelianFieldComponentDC[group,field,complexconjugate,fieldindices,repii,coup,boson,mu],
          If[Length[charge]>1,Print[Style["Warning : ambiguous contraction, please specify indices",Orange]];];
          dcres=dcres+ContstructNonAbelianNonFieldComponentDC[group,field,complexconjugate,fieldindices,repii,coup,boson,mu,If[Length[charge]>1,newside,side]]
        ];
...
PauliSigma/:PauliSigma[i_Integer,j_,k_Integer]^2:=Sum[PauliSigma[i,jjj,k],{jjj,1,2}];
PauliSigma/:PauliSigma[i_Integer,j_,k_Integer]PauliSigma[l_Integer,j_,m_Integer]:=Sum[PauliSigma[i,jjj,k]PauliSigma[l,jjj,m],{jjj,1,2}];
PauliSigma/:PauliSigma[i_Integer,k_Integer,j_]^2:=Sum[PauliSigma[i,k,jjj],{jjj,1,2}];
PauliSigma/:PauliSigma[i_Integer,k_Integer,j_]PauliSigma[l_Integer,m_Integer,j_]:=Sum[PauliSigma[i,k,jjj]PauliSigma[l,m,jjj],{jjj,1,2}];
...
HC[t_?((FieldQ[#]||TensQ[#])&)[ind___]] := HC[t][Sequence@@BarRep/@{ind}]/.BarRep->Identity;(* ! carefull with tensors as HC does not change the order of the indices (ex F[Index[SU2D,i], Index[SU2Dbar,j]]\[Rule]F^[Index[SU2Dbar,i],Index[SU2D,j]])! *)
...
(*HCanti[Power[a_, n_]] := Power[HCanti[a], HCanti[n]];*)
