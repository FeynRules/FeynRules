...
PrePutIndices[expr_] := Block[{BlockIt},
   expr //. {f_?(FieldQ)[ind___] :> PPInd[f, ind]}//.TensDot[u__,v_?(Head[#]=!=HCPI && Head[#]=!=HC&)][rr_, ss_] -> BlockIt[TensDot][u, v[rr,ss]]//.
    {TensDot[u__,HCPI[v_]][rr_, ss_] -> BlockIt[TensDot][u, HCPI[v],v[rr,ss]],TensDot[u__,HCPI[v_]][rr_, ss_] -> BlockIt[TensDot][u, HC[v],v[rr,ss]]} //. 
    $TensIndRules//.{BlockIt[TensDot][u__, HCPI[v_],v_[rr_, ss_]] -> TensDot[u, HCPI[v]][rr, ss],BlockIt[TensDot][u__, HC[v_],v_[rr_, ss_]] -> 
    TensDot[u, HC[v]][rr, ss],BlockIt[TensDot][u__, v_[ind__, rr_, ss_]] -> TensDot[u, v[ind]][rr, ss], BlockIt[TensDot][u__, v_[rr_, ss_]] -> TensDot[u, v][rr, ss]} //. 
    {del[fff_, Except[Index[___], mu_]] -> del[fff, Index[Lorentz, mu]]} /. Done -> Identity]; 
...
(*Print["xx1"];Print[InputForm[tmpexprlist]];*)
   ...
   If[getindlist==={1},joingetindlist={1};,
     joingetindlist = Union[getindlist];
     joingetindlist=(Union[#,SameTest->(#1===BarRep[#2]&)]&)/@joingetindlist;
     getindlist=(Union[#,SameTest->(#1===BarRep[#2]&)]&)/@getindlist;];
...
(*Print["xx2"];Print[InputForm[tmpexprlist]];*)
...
(*Print["xx3"];Print[InputForm[tmpexprlist]];*)
...
RemoveTensor[psi1_[inds1___],tt1_,tt2___,psi2_[inds2___]]:=Module[{i,j,temptt1,tind1,tind2,MyConjugate,MyTranspose, MyAdjointRep, MyTP},
   ...
   temptt1=PrePutIndices[If[MatchQ[tt1, _[__]], 
        Append[Append[tt1 /. {AdjointRep -> AdjointRepTemp, Conjugate -> MyConjugate, Transpose -> MyTranspose, TP -> MyTP},i],j] /. 
       {TensDot[t1_, tt__, ii_, jj_]:> TensDot[t1,tt][i,j], HCPI[tt__, ii_,jj_]:> HCPI[tt][ii,jj], MyTP[t_][iis__] :> MyTP[t[iis]],
        MyTranspose[tt__, ii_,jj_]:> MyTranspose[tt][ii,jj], MyConjugate[tt__, ii_, jj_] :> MyConjugate[tt][ii,jj]}, tt1[i,j]]
   ] /. Conjugate -> MyConjugate /. {MyConjugate[tt_[ii_,jj_]]:>MyConjugate[tt][jj,ii],MyTranspose[tt_][ii_,jj_]:>MyTranspose[tt][jj,ii], MyTP[t_[iis__]] :> TP[t][iis]};
   ...
   temptt1 * RemoveTensor[psi1[inds1,BarRep[tind1]],tt2,psi2[inds2,BarRep[tind2]]]];
...
RestoreFermionIndices[Except[_?(#=!=CC&)[__],psi1_?(FermionQ[#] === True &)],Except[_?(#=!=CC&)[__],psi2_?(FermionQ[#]===True&)]]:=Module[{indnames=$IndList[psi2],indnamesbar},
...
(*Mod. 01.25.2013. AA: If one of the psi is a ghost then return Dot[psi1,psi2] instead of Dot[psi1[], psi2[]] *)
   ...
   If[BarRep/@indnames=!=$IndList[psi1],Message[PutIndices::NoIndexMath]];
   indnames = Table[Append[indnames[[kk]],Symbol["i" <> ToString[kk]<>"$"<>ToString[$ModuleNumber]]],{kk,Length[indnames]}];
   indnamesbar = BarRep/@indnames;
   Dot[psi1@@indnamesbar,psi2@@indnames]];
...
(*Print["ttt7"];Print[InputForm[temp]];*)
(* Index restoration *)
...
(*Print["ttt8"];Print[InputForm[temp]];*)
    temp = PrePutIndices[temp];
(*Print["ttt8b"];Print[InputForm[temp]];*)
...
(*Print["ttt9"];Print[InputForm[temp]];*)
...
(*Print["ttt10"];Print[InputForm[temp]];*)

    If[flavorexpandQ, (*Print["Expanding flavors..."];*)
(*Print["ttt11"];Print[InputForm[temp]]; Print[flavorexpandlist];*)
...
(*Print["ttt9b"];Print[InputForm[temp]];*)
    temp = temp //. {Conjugate[tt_?(TensQ)][ind__] -> Conjugate[tt[ind]]};
