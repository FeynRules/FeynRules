...
SF2Components[exp_?(Head[#]=!=Plus&),mu_,alpha_,alphadot_] := Module[{tmp, resu, ll}, 
   ...
   tmp = exp/.Power[myexp_?(SuperfieldQ[#]===True&),n_]:>Table[myexp,n];
   tmp=If[ListQ[tmp],Expand[Times@@(ApplySuperfieldDefinitions[#]&/@tmp)], ApplySuperfieldDefinitions[tmp]];
...
ToIndexList[exp_]:=Block[{res,MyTens,aID,myeps},
(* pre-cleaning *)
  res=exp//.{Cos[x_]->x, Sin[x_]->x, Tan[x_]->x,IndexDelta->aID,Eps[Index[Lorentz,bla__],Index[Lorentz,bla2__],Index[Lorentz,bla3__],Index[Lorentz,bla4__]]:>myeps[Index[Lorentz,bla],Index[Lorentz,bla2],Index[Lorentz,bla3],Index[Lorentz,bla4]]};
...
(* Save the ordering of the expression and remove objects without indices *)
  ...
  res=res/.{HC[aa_]->aa,Conjugate[aa_]->aa,CC[aa_]->aa,aID[aa__]->Sequence[aa], PauliSigma[aa__]->Sequence[aa],K6bar[aa__]->Sequence[aa],K6[aa__]->Sequence[aa]};
  res=res/.Power[a_,n_? (IntegerQ[#]&)] :> Sequence@@Table[a,{Abs[n]}];
  res=res//.del[x_,mu_]->Sequence[x, Index[Lorentz,mu]];
  res=res/.FV[sa_, mu_]->Index[Lorentz, mu];
  res=res/.eeps[il_]:>Sequence@@il;
  res=res/.myeps[il__]:>Sequence@@(Index[Lorentz,#]&/@{il}); 
