...
(*Print["ttt1"];Print[InputForm[lagrangian]];Print[opts];*)
   out=PrepareLag[lagrangian, Sequence @@ opts, StandAlone->sa ];
(*Print["ttt2"];Print[InputForm[out]];*)
...
(*Print["ttt5"];Print[InputForm[templag]];Print[flavexplist];*)
      templag = PutIndices[templag, flavexp, flavexplist,nodef];
(*Print["ttt6"];Print[InputForm[templag]];*)
...
PrepareLag[lagrangian_, options___] := Block[{ 
      ...
      Which[$FlavExp === True, $FlavExpList = MR$FlavorList,
            ...
            True, $FlavExpList = Intersection[MR$FlavorList, Flatten[{$FlavExp}]];
                  $FlavExpList = Union[Flatten[(If[HasBarRepQ[#],{#,BarRep[#]},#,#]&)/@$FlavExpList]];
                  $FlavExp = True];
...
(*Print["ttt4"];Print[InputForm[templag]];*)
...
Off[Simplify::time];
  vtemp = If[$VersionNumber>8,TimeConstrained[Factor[Plus@@vtemp],100,Simplify[Plus@@vtemp,TimeConstraint->0.1]],Simplify[Plus@@vtemp,TimeConstraint->0.01]];(* by celine*)
On[Simplify::time];
