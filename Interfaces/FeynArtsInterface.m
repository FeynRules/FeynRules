...
SUNF /: SUNF[aa1_, cc_, aa2_]SUNF[bb1_,cc_,bb2_] := SUNF[aa2,aa1,bb2,bb1];
SUNF /: SUNF[cc_, aa2_,aa1_]SUNF[cc_,bb2_,bb1_] := SUNF[aa2,aa1,bb2,bb1];
SUNF /: SUNF[aa2_,aa1_, cc_]SUNF[bb2_,bb1_,cc_] := SUNF[aa2,aa1,bb2,bb1];
...
MakeIndexSum[expr_?(Head[#]=!=List &)]:=Block[{tmpexprlist,getindlist,IndexSum1,indx,term},
...
Options[WriteFeynArtsOutput] = {FlavorExpand -> False, IndexExpand-> False, MaxParticles -> Automatic, MinParticles -> Automatic, 
          ...
          Exclude4Scalars -> False, GenericFile -> True, DiracIndices->Automatic, LoopOrder->MR$Null, ApplyMomCons->True,Input->{}};
...
WriteFeynArtsOutput[{lags__}, options___] := Block[{vertlength,verttype,tempclass, vertlistFA,genFileOpt,diracIndOpt,couplRenameOpt,j,vertN=1, 
													...
													Massdone = {}, vertexlistFA, FlatDot, LoopOpt,CTlags,orderOpt,MyPat,mylags = {lags},CTcnt=0,tmp,tmppara,wfoit,inputvert},
      ...
      inputvert=Input/.{options}/.Options[WriteFeynArtsOutput];
...
(*Print["t22"];*)
		(*extra I factor to match the levicivita definition in FA*)
        vertexlistFA = LorentzContract[ExpandAll[vertexlistFA]]/.Eps[a__]:>I*Eps[a]/;Not[FreeQ[{a},Lorentz]];(*Print[InputForm[vertexlistFA]];*)
(*Print["t221"];*)
        vertexlistFA = vertexlistFA//.{Times[aa___,Eps[bb___,dd_,cc___],FV[ff_,dd_]]->Times[aa,Eps[bb,FV[ff],cc]]};
(*Print["t222"];*)
        vertexlistFA = OrderEps[vertexlistFA];
(*Print["t23"];*)
...
(*Print["FAformat 4"];*)
        (* Cleaning zero vertices *)
        vertexlistFA = Select[vertexlistFA,Not[MatchQ[DeleteCases[Flatten[#[[2]]],0],{}]]&];
...
FAStructure2[vertextype_, FAPartContent_, vertex_, fc_] := Module[{temp, ReturnConst, $HCHELAS, tempFApc, output,temp2},
	temp = vertex /. {T[Aaa_, Iii_, Jjj_] -> SUNT[Aaa, Iii, Jjj], T[Aaa_] -> SUNT[Aaa], f[Aaa_, Bbb_, Ccc_] -> SUNF[Aaa,Bbb,Ccc], ee -> EL, gs -> GS};
...
If[Length[Cases[temp,yx_?(FreeQ[#,Spin]&&FreeQ[#,Lorentz]&&FreeQ[#,SP]&)*newpat->yx]]>0&&kk>1,Print["test"];Print[pat];Print[ res];];
    ...
    Off[Simplify::time];
    temp = If[$VersionNumber>8,TimeConstrained[Factor[Expand[temp]],100,Simplify[Expand[temp],TimeConstraint->0.001]],Simplify[Expand[temp],TimeConstraint->0.001]];
    On[Simplify::time];
...
LorentzGenCouplings[vertextype_, FAPartContent_, vertex_, fc_] := Block[{temp, temp1, temp2, temp3, output, $HCHELAS, tempFApc, done, deltflavind, GaAlgebraDone},   
  ...
  temp = vertex; //. IndexDelta[Index[Spin, s_], Index[Spin, r_]] -> 1;
      ...
      temp = temp /. {T[Aaa_, Iii_, Jjj_] -> SUNT[Aaa, Iii, Jjj], T[Aaa_] -> SUNT[Aaa], f[aa_, bb_, cc_] -> SUNF[aa,bb,cc], ee -> EL, gs -> GS};
