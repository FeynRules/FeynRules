...
WriteUFO[lagrangians___, OptionsPattern[]] := Block[{lags = {lagrangians}, vertices, opts, message, decays = {},SignEpsInversion,
   (* internal and external parameters *)
   eparams = EParamList, iparams = IParamList, masslist = MassList, widthlist = WidthList,Zeroparam,MyRule,
   plist,
   (* Particles *)
    partlist = PartList,
   ...
   PY$UVLoopVertices = OptionValue[UVLoopCounterterms],
...
(* Preparation                                *)
(* * * * * * * * * * * * * * * * * * * * * * * *)
	SignEpsInversion={Eps[alor_,blor_,clor_,dlor_]:>-Eps[alor,blor,clor,dlor]/;Not[FreeQ[alor,Lorentz]]};

   (*remove evanescent operators and coefficients if present*)
   PY$UVVertices = PY$UVVertices/.NonCommutative[NLO$EO[__]][__]->0/.SignEpsInversion;
   PY$R2Vertices = PY$R2Vertices/.NLO$EvaA[__]->1/.SignEpsInversion;
   (*Simplify If*)
   PY$CTparameters=PY$CTparameters/.((Rule[#,#/.MR$Definitions]&)/@Union[Cases[PY$CTparameters,_If,\[Infinity]]]);
   (*Apply the restriction on the parameters*)
   PY$CTparameters=PY$CTparameters/.MR$Definitions;
   (*remove zero parameters*)
   Zeroparam=Cases[PY$CTparameters/.Rule->MyRule,MyRule[a_,0]]/.MyRule->Rule;
   PY$CTparameters=DeleteCases[PY$CTparameters/.Rule->MyRule,MyRule[a_,0]]/.MyRule->Rule;
   PY$UVVertices=PY$UVVertices/.Zeroparam;
   PY$UVLoopVertices=PY$UVLoopVertices/.Zeroparam;
   ...
   (* Set Global variables *)
    ...
    PY$UVLoopVertices=PY$UVLoopVertices/.((Rule[#,#/.MR$Definitions]&)/@Union[Cases[PY$UVLoopVertices,_If,\[Infinity]]]);
...
(* * * * * * * * * * * * * * * * * * * * * * * *)
   ...
   If[lags =!= {},
      ...
      *)
      ...
      vertices = Join @@ (FeynmanRules[#, Sequence @@ opts]& /@ lags)/.SignEpsInversion;
   ...
   (* Add the R2 and UV vertices, if present *)
    ...
    If[PY$UVLoopVertices =!= {},
       (*To avoid that IPL and IF are inside a conjugate*)
       PY$UVLoopVertices = PY$UVLoopVertices//.{Conjugate[aa_Plus]:>Conjugate/@aa,Conjugate[aa_Times]:>Conjugate/@aa}/.Conjugate[xx_IPL]:>xx/.Conjugate[If[xx_,a_,c_]]:>
         If[xx,Conjugate[a],Conjugate[c]];
       IsUFOAtNLO = True;
       PY$UVLoopVertices = AddPYR2UVTag[#, "UVloop"]& /@ PY$UVLoopVertices;
       vertices = Join[vertices, PY$UVLoopVertices];
      ];
...
Print[InputForm[Rule[#,GetDelta[#]]&/@Expand[FR$CTList]]];
       ...
       PY$UVVertices=PY$UVVertices/.PY$CTparameters//.{Conjugate[aa_Plus]:>Conjugate/@aa,Conjugate[aa_Times]:>Conjugate/@aa}/.Conjugate[xx_IPL]:>xx;
       PY$R2Vertices=PY$R2Vertices/.PY$CTparameters//.{Conjugate[aa_Plus]:>Conjugate/@aa,Conjugate[aa_Times]:>Conjugate/@aa}/.Conjugate[xx_IPL]:>xx;
       PY$CTparameters=Rule[#,GetDelta[#]]&/@FR$CTList;
       PY$CTparameters=Prepend[PY$CTparameters,Rule[invFREps,1/FR$Eps]];
      ];
...
If[$Debug,
  ...
  If[IsUFOAtNLO,
     ...
     {PY$UVLoopVertices,  PY$UVLoopCouplObjects} = BuildPYR2UVVertices[vertices, NLOcoupls, "UVloop"];
     {vertices,       PY$CouplObjects}   = BuildPYR2UVVertices[vertices, NLOcoupls, "1"];

     {PY$R2Vertices, PY$R2CouplObjects} = ProcessNLOVertices[PY$R2Vertices, PY$R2CouplObjects, PY$UVVertices, PY$UVCouplObjects, PY$UVLoopVertices, PY$UVLoopCouplObjects];
    (* Note that the variables now contains both R2 and UV, distinguished by a tag *)
  ];

$KEEP = PY$R2Vertices;
$KEEPUV = PY$UVVertices;
$KEEPUVLoop = PY$UVLoopVertices;
...
(* * * * * * * * * * * * * * * * * * * * * * * *)
 ...
 (*   (* Create restrictions.py *)
   ...
   If[PY$R2CouplObjects =!= {},
     ...
     WritePYCTCouplings[PY$R2CouplObjects,PY$CTparameters=!={}]
   ];
   ...
   If[PY$CTparameters =!={},
      WritePYCTParameters[PY$CTparameters]
   ];
