...
CreateCTParameterObjectEntry[paramdef_] := Block[{dicout},
   ...
   dicout = {{"name",         PYString[MakeString[paramdef[[1]]]]},
             {"type",         "'complex'"},
             {"value",        CreatePYLaurentSeriesDic[paramdef[[2]]]},
             {"texname",      StringJoin["'",ToString[paramdef[[1]]],"'"]}};
...
WriteCTParameterObject[file_, list_List] := Block[{
     classname = CreateObjectParticleName[list[[1,2]]]},

     WritePYObject[file, classname, "CTParameter", list];
     WriteString[file, "\n"];
];
