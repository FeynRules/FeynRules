...
CollectByTag[expi_] := Block[{
   DUM$NULL,
   tempexpi = Expand[expi + DUM$NULL*(1+PYR2UVTag["R2"]+PYR2UVTag["UV"]+PYR2UVTag["UVloop"])]
   },
...
CollectCouplingsByTag[CouplingObject[name_, coupl_]] := Block[{
    ...
    UVLoopCoupl = Cases[coupli, {PYR2UVTag["UVloop"], _}][[1,2]];

    Return[{CouplingObject[name, LOCoupl], R2CouplingObject[name, R2Coupl], UVCouplingObject[name, UVCoupl],  UVLoopCouplingObject[name, UVLoopCoupl]}];
]; 
...
GatherByLoopParticles[couplingobject_] := Block[{
   ...
   r2uvtype = If[head === R2CouplingObject, "R2", If[head === UVCouplingObject, "UV", "UVloop"]];
...
ProcessNLOVertices[R2verts_List, R2coupls_List, UVverts_List, UVcoupls_List, UVLoopverts_List, UVLoopcoupls_List] := Block[{
   R2V = R2verts,
   R2C = R2coupls, 
   UVV = UVverts,
   UVC = UVcoupls,
   ...
   UVLC = UVLC //. IPL -> CreatePYLoopParticles;
   ...
   UVLC = Flatten[GatherByLoopParticles /@ UVLC];
   ...
   PYNLO$UVLoopCouplingRename = CreateNewNamesForR2UVCouplingObject[UVLC, "UVloop"]; 
   ...
   UVLC = RenameNLOCoupling[#, PYNLO$UVLoopCouplingRename]& /@ UVLC;
   ...
   UVLV = AddLoopParticlesToVertex[#, UVLC, "UVloop"]& /@ UVLV;


   R2V = Join[R2V, UVV, UVLV];

   (* Finally, drop the superfluous entries for the coupling objects *)
   R2C = Drop[#, 2]& /@ R2C;
   UVC = Drop[#, 2]& /@ UVC;
   UVLC= Drop[#, 2]& /@ UVLC;
   R2C = KillDoubles[Join[R2C, UVC, UVLC]];
...
(*    If[FreeQ[factor,FR$Eps],
      ...
      If[GetDelta[mynamefin]=!=0, 
        FR$CTList=Append[FR$CTList,mynamefin];
        GetIntOrder[mynamefin]=GetIntOrder[Coefficient[factor,FR$Eps,0]];
        (myname+mynamepole/FR$Eps)*myipl,(mynamepole/FR$Eps)*myipl
      ]
    ]*)
    GetDelta[myname]=Expand[factor];
    FR$CTList=Append[FR$CTList,myname];
    GetIntOrder[myname]=GetIntOrder[factor];
    myname*myipl
  ]&/@value;
...
WritePYCTParameters[list_] := Block[{outfile},
   ...
   WriteCTParameterObject[outfile, #]& /@ (CreateCTParameterObjectEntry /@ list);
...
WritePYCTCouplings[list_,ctpar_:False] := Block[{outfile,tmplist},
...
(*Print["In WPYCTCoupl"];
  ...
  If[ctpar===True, 
    tmplist=list/.{1/FR$Eps->invFREps};
    tmplist=If[FreeQ[#,FR$Eps],#,#/.UVCouplingObject[name_,expr_]:>UVCouplingObject[name,ReplaceAll[expr,1/Denominator[expr]:>1/Simplify[Denominator[expr]]]/.{1/FR$Eps->invFREps}]]&/@tmplist;
    WriteCouplingObject[outfile, #]& /@ (CreateCTCouplingObjectEntry /@ tmplist),
...
PYFormLoopParticles[listi_List?(#=!={1}&)] := StringReplace[PYList[PythonFormatLoopParticles[listi //. PYLoopParticles -> List]],{"{"->"[","}"->"]"}];


PYFormLoopParticles[{1}]="[]";
