...
Stringify[expr_]:=StringReplace[ToString[expr /. {L$FV[k_, mu_] :> L$FV[mu, k]} //. L$Ga[5, inds___] :> L$Ga5[inds] //. PY$IndexForm /. ParamRules //. PY$InputForm, FortranForm], PY$OutputForm];


PythonForm[expr_] := Block[{string,stringify},
    
    (* Reset the counter for internl indices*)
    PY$IntIndexCoutner = 1; 
    
    string = Expand[expr];
    string = If[Head[string]===Plus,
      StringJoin[Riffle[If[Denominator[#]=!=1, "("<>Stringify[Numerator[#]]<>")/("<>Stringify[Denominator[#]]<>")","("<>Stringify[Numerator[#]]<>")"]&/@(List@@string),"+"]],
      If[Denominator[expr]=!=1,"("<>Stringify[Numerator[expr]]<>")/("<>Stringify[Denominator[expr]]<>")",Stringify[expr]]
    ];
