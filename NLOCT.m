...
DiracOpenChain[a___,b_+c_,d___][ind1_,ind2_]:=DiracOpenChain[a,b,d][ind1,ind2]+DiracOpenChain[a,c,d][ind1,ind2];
...
If[EvenQ[j-k],-1*FS$2,1*FS$2bis]*DiracOpenChain[a,Sequence@@CC/@Reverse[{b}]][Index[Dirac, i], Index[Dirac, k]]/;j>k&&j>i&&i<k;

DiracOpenChain/:DiracOpenChain[a___][Index[Dirac, j_], Index[Dirac, i_]]*DiracOpenChain[b___][Index[Dirac, j_], Index[Dirac, k_]]:=
If[EvenQ[j-i],-1*FS$3,1*FS$3bis]*DiracOpenChain[Sequence@@CC/@Reverse[{a}],b][Index[Dirac, i], Index[Dirac, k]]/;j>i&&j>k&&i<k;
...
-(FS$4bis*MatrixTrace[a,Sequence@@CC/@Reverse[{b}]])/;i<j;

DiracOpenChain/:DiracOpenChain[a___][Index[Dirac, i_], Index[Dirac, j_]]*DiracOpenChain[b___][Index[Dirac, i_], Index[Dirac, j_]]:= 
-(FS$4ter*MatrixTrace[b,Sequence@@CC/@Reverse[{a}]])/;i>j;

DiracOpenChain/:DiracOpenChain[a___][Index[Dirac, i_], Index[Dirac, i_]]:=-FS$4*MatrixTrace[a];


CC[NonCommutative[ChiralityProjector[pm_]]] :=NonCommutative[ChiralityProjector[pm]];
CC[NonCommutative[a:_DiracMatrix|_DiracSlash]] := -NonCommutative[a];
...
AmpRenaming[amp1_,amp2_]:=Block[{artmp,arden1,arden2,arext1,arext2,arv1,arv2,arrep},
...
DPrint[Style["in AR, same deno",Green]];
...
For[arkk=1,arkk<=Length[arv2],arkk++,
  ...
  artmp=amp2/.arrep;
  If[artmp===amp1,Return[Reverse/@arrep]];
];
(*DPrint[InputForm[amp1]];
DPrint[InputForm[amp2]];*)
(*no match found*)
False
];
NLO$currentvert={};
...
Print["eq1"];Print[SessionTime[]];
     interres = If[Simplify[TheMass[fullUV[[kuv,1,1,1]]],Assumptions->assumlist]===0&&Simplify[TheMass[fullUV[[kuv,1,2,1]]],Assumptions->assumlist]===0,
                  fr,fr*TheMass[fullUV[[kuv,1,2,1]]]+fsl]/.su3rep;
     ColCons2pt[interres];
     interres = EpsSerie[interres/.SP[FourMomentum[Incoming,2],FourMomentum[Incoming,2]]->TheMass[fullUV[[kuv,1,2,1]]]^2];
     eqlist=Append[eqlist,interres];
Print["eq2"];Print[SessionTime[]];
...
GetR2[amp_,next_,UVfin_,evenOnly_,maxDim_,gaugef_]:=Block[{temp,loopmom,nden,den,num,kk,extmom,extmom2,intmom,lmcoef,DisFC,DisMT,DisDS,DisNC,nlmom,x,y,rl,lr,delta,noextpos,feynp,PW,pairtmp,
                    perm,mep,mep2,fvp,nq,LoopInt,Epsi,feynpar,RemoveHDim,FChain,tempUV,FullLoopInt,findfp,fplist,fp1,numtmp,kkpar,tmppar,exprm,MF,momshift},
DPrint["In EFT GetR2"];
...
DPrint[InputForm[Cases[num,_?(Not[FreeQ[#,Dirac]]&)]]];
  num=Expand[num];
DPrint["after total expand 1 "<>ToString[SessionTime[]]];DPrint[Length[num]];
...
(*DPrint[InputForm[loopmom-extmom]];DPrint[InputForm[delta]];*)
  ...
  DPrint["after switch delta extmom "<>ToString[SessionTime[]]];
  ...
  Print["before rm H dim "<>ToString[SessionTime[]]];DPrint[Length[num]];
...
DPrint[Length[num]];
  DPrint["after rm H dim "<>ToString[SessionTime[]]];DPrint[Length[num]];
  (*num=num/.loopmom ->loopmom-extmom;*)
DPrint["after switch (momentum shift) "<>ToString[SessionTime[]]];DPrint[Length[num]];
...
(*to do condition on maxdim and nden for no mom shift or return 0 directly if[nden-maxdim\[Equal]0,no shift,If[nedn-maxdim>0,0]], feynmangauge only?*)
  ...
  exprm[x_]:=Block[{texprm},
    ...
    texprm=Expand[x/.loopmom ->loopmom-If[extmom===0,0,FourMomentum[momshift]](*extmom*)/.{MetricTensor->ME,FourVector->FV,ScalarProduct->SP,FChDist->FChDist2,DTrDist->DTrDist2}];
...
DPrint["before x integration "<>ToString[SessionTime[]]];
...
R2atClass[Ampl_,INTopo_,verttype_,lab_,kept_,qcd_,UVfinite_,fg_,evenDonly_,maxD_,qcdord_]:=Block[{res,tmp,tmp2,top,gen,rf,rl,fc,scalar,intern,nExtC,ll,
  notalreadydone, repl,ampkk}, 
  res=0;
...
DPrint["current vert is ok"];
      repl=False;
      ampkk=Length[NLO$ampR2];
(*DPrint[InputForm[Ampl[[kk,3]]]];*)
      While[Length[repl]<1&&ampkk>=1, repl=AmpRenaming[NLO$ampR2[[ampkk,1]],Ampl[[kk,3]]];DPrint[repl]DPrint[ampkk];ampkk--;];
      If[repl===False,notalreadydone=True;,notalreadydone=False;];
      ,
Print["current vert is not ok"];
      (*first time with this vertex type*)
      notalreadydone=True;NLO$currentvert=verttype;NLO$ampR2={};
   ];
    If[True(*notalreadydone*),
      tmp=If[fg&&(maxD<=4),GetR2[Ampl[[kk]],Length[verttype],UVfinite],GetR2[Ampl[[kk]],Length[verttype],UVfinite,evenDonly,maxD,fg]];
      ...
      (*already computed*)
DPrint[Style["already done",Orange]];
...
If[Not[FreeQ[tmp,_IndexDelta]],DPrint[Style[InputForm[tmp],Green]];];
          ...
          If[qcd,
            ...
            tmp2= Total[Table[Coefficient[tmp2,GS,nn]*GS^nn,{nn,Max[{2,(Append[Cases[qcdord,{nExtC[[1]],nExtC[[2]],a_}->a],0][[1]]+2)}],4}]];
          ];
          res=res+tmp2;
        ];
      ];
...
R2vertlist[vertsum_]:=Block[{vertList,lab,ver,pos,ss,v,tt,ind,indrep,f,indlist,ind2,ind3,xx,ll,kk,nn,ff,mm,UVmass,deltarule,indl,aa,bb,s1,s2,extsum,wft,inveps,res,ftmpind,nferm},
  vertList={};
  ...
  For[kk=1,kk<=Length[res],kk++,
    ...
    lab=Join[lab,Cases[res[[kk]],FermionChain[NonCommutative[SpinorType[f_][-FourMomentum[Incoming,ind_]|FourMomentum[Incoming,ind_],__]],xx__]->{f,ind,ftmpind},\[Infinity]]];
    nferm=1;
    While[Not[FreeQ[lab,ftmpind]],lab[[Sequence@@Position[lab,ftmpind][[1]]]]=2nferm-1;nferm++;];
    lab=Join[lab,Cases[res[[kk]],FermionChain[xx__,NonCommutative[SpinorType[f_][-FourMomentum[Incoming,ind_]|FourMomentum[Incoming,ind_],__]]]->{f,ind,ftmpind},\[Infinity]]];
    nferm=1;
...
ModelR2[mod_,gen_,lab_,kept_,qcd_,zerom_,assumpt_,UVCT_,comas_,ctpa_,no4SCT_,fga_,evenD_,mdim_,gvlist_,susy_,mixon_,mzs_]:=Block[{vertlist,genver,kkpl,topo,INTopo,Ampl,tmp,tmpCT,x,ind,indl,rl1,rlf,tdrl,a,b,temp,ckt,topoCT,INTopoCT,AmplCT,CTSol,
...
(*Print[InputForm[AmplCT]];
Print["before CTatClass"];*)
...
(*DPrint[InputForm[Ampl]];*)
    ...
    tmp=R2atClass[Ampl,INTopo,genver[[kkpl]],lab,kept,qcd,UVCT,fga,evenD,mdim,qcdorders];
...
(*Length[tmp]=0*)
        If[Length[tmpCT]>0&&Length[genver[[kkpl]]]===2,
          tmpCT[[All,3]]=(ReplaceRepeated[#/.rl1/.tdrl/.IndexSum->Sum/.rlf,SumOver[ind_?((FreeQ[#,Gluon]&&FreeQ[#,Colour])&),x_Integer]*a__:>dum1 Total[Table[(Times[a])/.{ind->kkpl},{kkpl,1,x}]]]&)/@tmpCT[[All,3]];
...
MixingOnly->False,MZscheme->False};
...
assume,uvos,NC$cms,CTparam,NC$ctp,no4S,NC$out,feynmang,evenO,mdimen,genvertl,susy,mixo,mzsch},
...
If[NC$cms,NLO$CMS=True;,NLO$CMS=False;];
  ...
  NC$totlist=ModelR2[mod,gen,LabInt,KeptInd,NC$qcd,NC$zerom,assume,uvos,NC$cms,NC$ctp,no4S,feynmang,evenO,mdimen,genvertl,susy,mixo,mzsch];
